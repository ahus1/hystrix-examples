== Aggregating Hystrix runtime information in a cluster with Turbine
...
Turbine uses the same confiuguration mechanism as Hystrix: Archaius. See chapter "<<archaius>> for more information. The following confiuguration has already been set up for you in `archaius.properties`:
...
To have you application publishing the information on the URL suffix above, you'll need to add a servlet to your application. This has already been packaged for you. You'll just need to include as a dependency of your application in `pom.xml`:
...
The you register it in `web.xml` as a servlet:
